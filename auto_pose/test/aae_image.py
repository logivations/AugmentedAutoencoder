...
start_time = time.time()
codebook, dataset = factory.build_codebook_from_name(experiment_name, experiment_group, return_dataset=True)
# codebook = factory.build_codebook_from_name(experiment_name, experiment_group, return_dataset=False)
end_time = time.time()
print("codebook generation: ", str(end_time - start_time))
...
with tf.Session() as sess:
    ...
    for i in range(1, 8):
        ...
        video_path = '/home/logi/Downloads/test_images/test_' + str(i) + '.MOV'
        codec = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter(os.path.join(workspace_path, "out_" + os.path.basename(os.path.splitext(video_path)[0]) + ".avi"), codec, 30, (512, 256))
        cap = cv2.VideoCapture(video_path)
        successive_falses = 0
        while cap.isOpened() and successive_falses < 30:
            ...
            if ret:
                ...
                start_time = time.time()
                R, angles = codebook.nearest_rotation(sess, im)
                end_time = time.time()
                print("inference time: ", int(1000 * (end_time - start_time)) / 1000., "    fps: ", int(1 / (end_time - start_time)))
                print(R)
                pred_view = dataset.render_rot( R,downSample = 1)

                pred_view = cv2.cvtColor(pred_view, cv2.COLOR_BGR2GRAY)
                print(pred_view.shape)
                print(im.shape)

                pred_view = np.stack((pred_view, pred_view, pred_view),axis=2)
                im = np.stack((im.squeeze(axis=2), im.squeeze(axis=2), im.squeeze(axis=2)), axis=2)
